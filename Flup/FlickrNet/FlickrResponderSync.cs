    ...
    public static partial class FlickrResponder
    {
        ...
        private static string DownloadData(string method, string baseUrl, string data, string contentType, string authHeader)
        {
            Func<string> f = () =>
            {
                ...
                using (var client = new WebClientEx.WebClientEx() { ConnectionLimit = 20, Expect100Continue = false })
                {
//                    client.ConnectionLimit = 1; // TODO nejak dynamicky nastavovat
            ...
            //        //if (!string.IsNullOrEmpty(contentType)) client2.Headers.Add("Content-Type", contentType);
            //        //if (!string.IsNullOrEmpty(authHeader)) client2.Headers.Add("Authorization", authHeader);

            //        if (!string.IsNullOrEmpty(contentType)) client2.DefaultRequestHeaders.TryAddWithoutValidation("Content-Type", contentType);
            //        if (!string.IsNullOrEmpty(authHeader)) client2.DefaultRequestHeaders.TryAddWithoutValidation("Authorization", authHeader);
            ...
            //            var p = new KeyValuePair<string, string>("", data);
            //            pairs.Add(p);
            //            var content = new FormUrlEncodedContent(new[]
            //{
            //    new KeyValuePair<string, string>("", data)
            //});
            //            var httpResponseMessage = client2.PostAsync(baseUrl, content).Result;
            ...
            //            //var h = client2.PostAsync(baseUrl, )

            //            //var httpResponse = client2.PostAsync(baseUrl,  .SendAsync(request).Result; // .UploadString(baseUrl, data);

            //            //var a = httpResponse.Content.ReadAsStringAsync().Result;
            //            //return a;
            //        }
            //        return client2.GetStringAsync(baseUrl).Result;
            //    }
            //};
